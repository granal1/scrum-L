            ...
            <div class="card shadow">
                <div class="card-header">
                    <div class="d-grid gap-2 d-md-flex align-items-center justify-content-between">
                        @auth
                            @can('create', \App\Models\OutputFiles\OutputFile::class)
                                <a class="btn btn-outline-success btn-sm" href="{{route('output_files.create')}}">Добавить</a>
                            @endcan
                            @endauth
                        <h4 class="d-inline-block">Журнал учета исходящих документов</h4>
                            ...
                            <a class="btn btn-outline-danger btn-sm d-md-none" type="button" href="{{route('output_files.index')}}">Сброс</a>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col">
                            <table class="table table-sm table-hover table-striped">
                                ...
                                <tr class="collapse @if(!empty($old_filters)) show @endif" id="collapseExample">
                                    <form action="{{ route('output_files.index') }}" method="get">
                                        <td class="d-none d-md-table-cell"><a class="btn btn-outline-danger btn-sm" type="button" href="{{route('output_files.index')}}">Сброс</a></td>
                                      <td colspan="4"></td>
                                        <td>
                                ...
                                @forelse($output_files as $output_file)
                                    <tr  onclick="window.location='{{ route('output_files.show', $output_file->id) }}';">
                                        <td class="d-none d-md-table-cell">{{$output_file->incoming_at ? date('d.m.Y', strtotime($output_file->incoming_at)) : null}}</td>
                                        <td class="d-none d-md-table-cell">{{$output_file->incoming_number}}</td>
                                        <td class="d-none d-md-table-cell">{{$output_file->incoming_author}}</td>
                                        <td class="d-none d-md-table-cell">{{$output_file->number}}</td>
                                        <td class="d-none d-md-table-cell">{{$output_file->date ? date('d.m.Y', strtotime($output_file->date)) : null}}</td>
